        ...
        if (response != null && response['status'] == "success") {
          ...
          if (sharedPref.getString("type") == "admin") {
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(builder: (context) => const HomeAdmin()), // استخدام const مع الـ Widget
            );
          } else if (sharedPref.getString("type") == "owner" || sharedPref.getString("type") == "renter") {
            // ممكن تضيف هنا تحقق لو فيه أنواع تانية غير owner عشان يروح على home_owner
            // أو تعمل HomeRenter لو ليها شاشة مختلفة
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(builder: (context) => const HomeOwner()), // استخدام const مع الـ Widget
            );
          } else {
             // لو نوع المستخدم غير معروف أو لم يتم إرجاعه بشكل صحيح
             setState(() {
               _errorMessage = "نوع المستخدم غير معروف. يرجى الاتصال بالدعم الفني.";
             });
          }
        } else {
          // لو الـ status مش "success" يبقى فيه خطأ
          String message = "فشل تسجيل الدخول. يرجى التأكد من البريد وكلمة المرور.";
          if (response != null && response['message'] != null) {
            message = response['message']; // استخدام الرسالة اللي جاية من الباك إند
          }
          setState(() {
    ...
    return Scaffold(
      ...
      body: Center(
        child: isloading == true
            ? const Center(child: CircularProgressIndicator()) // استخدام const هنا
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      TextFormField(
                        ...
                        obscureText: true,
                        validator: (val) {
                          return validInput(val!, 6, 30); // يفضل 6 أحرف على الأقل للباسورد
                        },
                        decoration: InputDecoration(
                      ...
                      ElevatedButton(
                        ...
                        child: const Text( // استخدام const
                          "تسجيل الدخول",
                          style: TextStyle(
                            color: Colors.white,
                            fontSize: 16,
                      ...
                      TextButton(
                        onPressed: () {
                          Navigator.pushReplacement(
                            context,
                            MaterialPageRoute(
                              builder: (context) =>  RegisterScreen(), // استخدام const
                            ),
                          );
                        },
                        child: Text(
